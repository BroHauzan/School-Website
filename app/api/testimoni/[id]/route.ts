...
import { getTestimoniById, updateTestimoni, deleteTestimoni } from "@/lib/testimoni-server";
...
export async function PATCH(request: Request, { params }: Ctx) {
  try {
    ...
    const updated = await updateTestimoni(id, input);
...
export async function DELETE(request: Request, { params }: Ctx) {
  try {
    ...
    await requireAdmin();
    const { id } = await params;
    const prev = await deleteTestimoni(id);
    if (!prev) return NextResponse.json({ error: "Testimoni tidak ditemukan." }, { status: 404 });
    revalidatePath("/", "layout");
