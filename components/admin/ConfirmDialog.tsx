    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/60 p-6" onClick={() => !busy && setOpen(false)}>
          <div className="w-full max-w-sm rounded-lg bg-paper p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-display text-xl text-ink">{title}</h3>
            <p className="mt-2 text-sm leading-relaxed text-muted">{desc}</p>
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" disabled={busy} onClick={() => setOpen(false)} className="rounded-full border border-navy/20 px-5 py-2 text-sm text-navy disabled:opacity-50">Batal</button>
