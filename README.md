...
- **Prestasi** dari koleksi Firestore `prestasi`: `year`, `dateISO` (opsional, "YYYY-MM-DD", default hari ini saat input baru) + `dateLabel` hasil format, `scope` (Kabupaten/Provinsi/Nasional/Internasional), `title`, `peraih[]` (`nama` + `kelas` opsional per orang), `published`. Satu prestasi bisa punya banyak peraih (baris form bisa ditambah/dihapus); kolom `who`/`kelas` lama otomatis dikonversi saat dibaca. Diinput lewat `/admin/prestasi`. Kosong → blok coming soon.
- **Bagikan prestasi (share as image)** — tiap baris tabel `/prestasi` punya tombol bagikan **icon-only** di kolom paling kanan (lebar tetap `w-14`, kotak 40×40px). Di desktop tombol tersembunyi (`opacity-0`) dan muncul saat baris di-hover (`group-hover`) atau difokus keyboard; di perangkat sentuh selalu terlihat redup (`pointer-coarse:opacity-60`). Klik → kartu 1080×1350 (rasio 4:5, aman untuk feed IG/WA) dirender off-screen oleh `components/prestasi/SharePrestasiCard.tsx`, di-capture jadi PNG via `html-to-image` (di-lazy-load supaya tidak masuk bundle awal), lalu tampil di modal dengan tombol **Unduh PNG** dan **Bagikan** (Web Share API, muncul hanya bila browser mendukung berbagi file). Font di-embed setelah `document.fonts.ready`; kartu memakai style inline literal (bukan utility opacity Tailwind) agar hasil PNG konsisten di Safari. Background kartu (`components/prestasi/ShareCardBackground.tsx`): radial glow point-light dari pusat logo + grain tile alpha-baked + lengkung dekoratif kanan atas + watermark mata owl + vignette. Ukuran, warna badge, origin/radius glow, dan intensitas layer diatur di `components/prestasi/share-card-theme.ts`. **Catatan capture:** `mix-blend-mode` tidak ikut ter-capture html-to-image — grain karena itu ditulis sebagai piksel ber-alpha di tile PNG, bukan layer blend.
- **Testimoni** dari koleksi Firestore `testimoni`: `quote` (10-500 karakter), `name`, `role`, `order` (urutan carousel, kecil = depan), `published`. Diinput lewat `/admin/testimoni`. Section homepage menampilkan carousel manual TANPA autoplay — kartu bergeser saat tombol panah / dot / swipe sentuh / keyboard ArrowLeft-Right dipencet (transisi halus, mati saat prefers-reduced-motion); 3 kartu terlihat di desktop, 2 tablet, 1 HP; bila koleksi kosong → fallback 3 testimoni statis bawaan.
